...
namespace Generic
{
    StandardGrid::StandardGrid(const vector_d &input) : _tj(input)
    {
        ...
        for(size_t j = 0; j <= _p; j++){
            for(size_t n = 0; n <= _p; n++){
                if(n==j) continue;
                _Dij[j][j] += 1. / (_tj[j] - _tj[n]);
            }
        }
        ...
        for(size_t j = 0; j <= _p; j++){
            for(size_t i = 0; i <= _p; i++){
                if(i == j) continue;
                _Dij[j][i] = 1. / _tj[j] - _tj[i];
                for(size_t n = 0; n <= _p; n++){
                    if(n == i || n == j) continue;
                    _Dij[j][i] *= (_tj[i] - _tj[n]) / (_tj[j] - _tj[n]);    
                }
            }
        }
    }
    StandardGrid::StandardGrid(const std::function<double(size_t, size_t)> &fnc, size_t p) : _p(p)
    {
        ...
        // std::sort(_tj.begin(), _tj.end()); // Non serve dato che all'inizio ci sono i due if che controllano che la funzione sia corretta
    ...
    double StandardGrid::interpolate(double t, const vector_d &fj, size_t start, size_t end, STRATEGY str) const
    {
        ...
        case STRATEGY::FBF: {
            ...
            double sum = 0.;
            for (size_t i = start, j = 0; i <= end; i++, j++) {
                sum += poly_weight_fbf(t, j, monic) * fj[i];
            }
            return sum;
            break;
        }
        case STRATEGY::SBF: {
            ...
            double sum = 0;
            for (size_t i = start, j = 0; i <= end; i++, j++) {
                sum += poly_weight_sbf(t, j, den) * fj[i];
            }
            return sum;
        }
        default:
    ...
    double StandardGrid::interpolate_der(double t, const vector_d &fj, size_t start, size_t end, STRATEGY str) const
    {
        ...
        case STRATEGY::FBF: {
            ...
            double sum = 0.;
            for (size_t i = start, j = 0; i <= end; i++, j++) {
                sum += poly_weight_fbf_der(t, j, monic) * fj[i];
            }
            return sum;
            break;
        }
        case STRATEGY::SBF: {
            ...
            double sum = 0;
            for (size_t i = start, j = 0; i <= end; i++, j++) {
                sum += poly_weight_sbf_der(t, j, den) * fj[i];
            }
            return sum;
        }
        default:
    ...
    double StandardGrid::poly_weight_fbf(double t, size_t j) const
    {
        if (std::abs(t - _tj[j]) < 1.0e-15) return 1.;
    ...
    double StandardGrid::poly_weight_fbf(double t, size_t j, double monic) const
    {
        if (std::abs(t - _tj[j]) < 1.0e-15) return 1.;
        return monic * _lambdaj[j] / (t - _tj[j]);
    }

    double StandardGrid::poly_weight_sbf(double t, size_t j) const
    {
        if (std::abs(t - _tj[j]) < 1.0e-15) return 1.;
    ...
    double StandardGrid::poly_weight_sbf(double t, size_t j, double den) const
    {
        if (std::abs(t - _tj[j]) < 1.0e-15) return 1.;
        return _lambdaj[j] / (t - _tj[j]) / den;
    }
    ...
    {
        if (t < -1 || t > 1) {
            throw std::domain_error("[StandardGrid::poly_weight]: t=" + std::to_string(t) + " \\notin [-1, +1]");
        }
        double res = 0;
        for (size_t i = 0; i <= _p; i++) {
    ...
    double StandardGrid::poly_weight_fbf_der(double t, size_t j) const
    {
        if (t < -1 || t > 1) {
            throw std::domain_error("[StandardGrid::poly_weight]: t=" + std::to_string(t) + " \\notin [-1, +1]");
        }
        ...
        for (size_t i = 0; i <= _p; i++) {
            if (fabs(t - _tj[i]) < 1.0e-15) return _Dij[j][i];
            res += _Dij[j][i] * poly_weight_fbf(t, i);
        }
        return res;
    }

    double StandardGrid::poly_weight_fbf_der(double t, size_t j, double monic) const
    {
        if (t < -1 || t > 1) {
            throw std::domain_error("[StandardGrid::poly_weight]: t=" + std::to_string(t) + " \\notin [-1, +1]");
        }
        ...
        for (size_t i = 0; i <= _p; i++) {
            if (fabs(t - _tj[i]) < 1.0e-15) return _Dij[j][i];
            res += _Dij[j][i] * poly_weight_fbf(t, i, monic);
        }
        return res;
    }

    double StandardGrid::poly_weight_sbf_der(double t, size_t j) const
    {
        if (t < -1 || t > 1) {
            throw std::domain_error("[StandardGrid::poly_weight]: t=" + std::to_string(t) + " \\notin [-1, +1]");
        }
        ...
        for (size_t i = 0; i <= _p; i++) {
            if (fabs(t - _tj[i]) < 1.0e-15) return _Dij[j][i];
            res += _Dij[j][i] * poly_weight_sbf(t, i);
        }
        return res;
    }

    double StandardGrid::poly_weight_sbf_der(double t, size_t j, double den) const
    {
        if (t < -1 || t > 1) {
            throw std::domain_error("[StandardGrid::poly_weight]: t=" + std::to_string(t) + " \\notin [-1, +1]");
        }
        ...
        for (size_t i = 0; i <= _p; i++) {
            if (fabs(t - _tj[i]) < 1.0e-15) return _Dij[j][i];
            res += _Dij[j][i] * poly_weight_sbf(t, i, den);
        }
        return res;
    }
    ...
    {
        if (end - start != _p) {
            throw std::invalid_argument("[StandardGrid::apply_D]: cannot apply derivative matrix to partial vector.");
        }
