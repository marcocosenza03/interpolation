...
int test_exp_function()
{
   ...
   std::FILE *fptr = std::fopen("StandardGrid_interpolation_generic.dat", "w");
...
int test_exp_function()
{
   ...
   double m_n = 0, md_n = 0., md_n_v2 = 0.;
   double m_1 = 0, md_1 = 0., md_1_v2 = 0.;
   double m_2 = 0, md_2 = 0., md_2_v2 = 0.;
   ...
   for (size_t i = 0; i < n; i++) {
      ...
      const double exact_d   = testfunction_d(x);
      const double inter_d_n = grid.interpolate_der(x, v, 0, grid._p, ST::NAIVE);
      const double inter_d_1 = grid.interpolate_der(x, v, 0, grid._p, ST::FBF);
      const double inter_d_2 = grid.interpolate_der(x, v, 0, grid._p, ST::SBF);
      const double inter_d_n_v2 = grid.interpolate_der_v2(x, v, 0, grid._p, ST::NAIVE);
      const double inter_d_1_v2 = grid.interpolate_der_v2(x, v, 0, grid._p, ST::FBF);
      const double inter_d_2_v2 = grid.interpolate_der_v2(x, v, 0, grid._p, ST::SBF);

      m_n  = std::max(m_n, std::abs(exact - inter_n));
      md_n = std::max(md_n, std::abs(exact_d - inter_d_n));
      md_n_v2 = std::max(md_n_v2, std::abs(exact_d - inter_d_n_v2));

      m_1  = std::max(m_1, std::abs(exact - inter_1));
      md_1_v2 = std::max(md_1_v2, std::abs(exact_d - inter_d_1_v2));

      m_2  = std::max(m_2, std::abs(exact - inter_2));
      md_2_v2 = std::max(md_2_v2, std::abs(exact_d - inter_d_2_v2));
   ...
   std::printf("Naive Max difference in interpolation: %.6e\n", m_n);
   std::printf("Naive Max difference in interpolation derivatives: %.6e %.6e\n", md_n, md_n_v2);

   std::printf("FBF Max difference in interpolation: %.6e\n", m_1);
   std::printf("FBF Max difference in interpolation derivatives: %.6e %.6e\n", md_1, md_1_v2);

   std::printf("SBF Max difference in interpolation: %.6e\n", m_2);
   std::printf("SBF Max difference in interpolation derivatives: %.6e %.6e\n", md_2, md_2_v2);
